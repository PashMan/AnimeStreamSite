...
const Home: React.FC = () => {
  ...
  // Lazy loading states
  const [shouldLoadOngoing, setShouldLoadOngoing] = useState(false);
  ...
  const [shouldLoadForum, setShouldLoadForum] = useState(false);
  ...
  // Lazy Load Effects
  useEffect(() => {
    if (shouldLoadTrending && trendingAnimes.length === 0) {
      fetchAnimes({ order: 'popularity', limit: 20 }).then(setTrendingAnimes);
    }
  }, [shouldLoadTrending]);

  useEffect(() => {
    if (shouldLoadOngoing && newAnimes.length === 0) {
      fetchAnimes({ order: 'ranked', status: 'ongoing', limit: 20 }).then(setNewAnimes);
    }
  }, [shouldLoadOngoing]);

  useEffect(() => {
    if (shouldLoadNews && news.length === 0) {
      fetchNews().then(setNews);
    }
  }, [shouldLoadNews]);

  useEffect(() => {
    if (shouldLoadSchedule && schedule.length === 0) {
      fetchCalendar().then(setSchedule);
    }
  }, [shouldLoadSchedule]);

  useEffect(() => {
    if (shouldLoadForum && forumTopics.length === 0) {
      db.getForumTopics(undefined, undefined, 5, 'news').then(setForumTopics);
    }
  }, [shouldLoadForum]);
  ...
  return (
    <div className="space-y-24 pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-28">
        {/* Ongoing Section */}
        <LazySection onVisible={() => setShouldLoadOngoing(true)}>
        ...
        {/* Trending Section */}
        <LazySection onVisible={() => setShouldLoadTrending(true)}>
          <div className="flex flex-col md:flex-row justify-between gap-6 mb-10">
            <div className="flex items-center gap-4">
        ...
        <LazySection onVisible={() => setShouldLoadSchedule(true)} className="relative z-20"> 
          ...
          <div className="hidden md:grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-4">
            {schedule.length > 0 ? schedule.map((day) => (
              <div key={day.day} className="relative h-[160px] group">
                ...
                Array.from({length: 7}).map((_, i) => (
                    <div key={i} className="h-[160px] bg-white/5 rounded-[1.5rem] animate-pulse"></div>
                ))
            )}
          </div>
        ...
        <LazySection onVisible={() => setShouldLoadNews(true)} className="relative z-10">
        ...
        <LazySection onVisible={() => setShouldLoadForum(true)} className="relative z-10">
             <div className="flex items-center justify-between mb-8">
                <div className="flex items-center gap-4">
