export const onRequest = async (context: any) => {
  ...
  // ВАЖНО: Kodik API может не понимать закодированные запятые (%2C)
  // Поэтому мы декодируем URL перед отправкой
  const finalUrl = decodeURIComponent(targetUrl.toString());
  ...
  try {
    ...
    return new Response(JSON.stringify({ error: String(e), targetUrl: finalUrl }), { 
      status: 500,
      headers: { 
        'Content-Type': 'application/json',
        'Access-Control-Allow-Origin': '*'
