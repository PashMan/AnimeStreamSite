...
// Use direct URL since Kodik blocks server IPs (Cloud Run)
...
export const checkKodikAvailability = async (shikimoriIds: string[]): Promise<Set<string>> => {
  ...
  // Запрашиваем каждый ID отдельно, чтобы избежать проблем с запятыми в URL при проксировании
  const promises = shikimoriIds.map(async (id) => {
    try {
      const data = await fetchApi(`${BASE_URL}/search?token=${KODIK_TOKEN}&shikimori_id=${id}`);
