...
export const KODIK_TOKEN = "b3b563060d02ee000ca18740b7842ca0";
// Direct access to avoid proxy issues
export const BASE_URL = "https://kodikapi.com"; 
...
export const checkKodikAvailability = async (shikimoriIds: string[]): Promise<Set<string>> => {
  ...
  for (let i = 0; i < shikimoriIds.length; i += chunkSize) {
    const chunk = shikimoriIds.slice(i, i + chunkSize);
    try {
      const idsString = chunk.join(',');
      const data = await fetchApi(`${BASE_URL}/search?token=${KODIK_TOKEN}&shikimori_id=${idsString}`);
      
      if (data?.results) {
        data.results.forEach((item: any) => {
          if (item.shikimori_id) {
            availableIds.add(item.shikimori_id.toString());
          }
        });
      }
    } catch (e) {
      console.error("Error fetching Kodik chunk:", e);
    }
  }
